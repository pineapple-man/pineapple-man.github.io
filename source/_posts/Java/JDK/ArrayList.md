...
:sparkles:`DEFAULT_CAPACITY`规定了底层数组最大容量为10，当不断添加元素后，需要进行`ArrayList`的扩容机制
...
:notes:关于Java GC这里需要特别说明一下，**有了垃圾收集器并不意味着一定不会有内存泄漏**。对象能否被GC的依据是是否还有引用指向它，上面代码中如果不手动赋`null`值，除非对应的位置被其他元素覆盖，否则原来的对象就一直不会被回收
