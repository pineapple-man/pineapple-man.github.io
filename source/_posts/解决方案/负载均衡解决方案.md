...
:book:负载均衡分为软件负载均衡和硬件负载均衡，前者的代表是`LVS`，后者则是均衡服务器（比如`F5`），常见的负载均衡架构如下
...
{%endalert%}
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/一致性哈希.webp %}
#### 算法原理
:sailboat:具体的算法原理如下：
1. 存在这样一个哈希环，一个长度为 32 位的哈希环，环上存在一个$2^32$个哈希桶，通过哈希函数能够将不同的请求映射到哈希环上的某个桶中，一个哈希环如下：
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/哈希环.webp %}

2. 每一个服务节点都通过哈希函数映射到换上的某个桶中，称为 `Node`。每个 Node 管理从其当前桶节点开始到下一个存在服务的桶节点之前的所有空节点，如下图所示
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/后端节点映射.webp %}
...
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/请求映射.webp %}
#### 节点宕机
...
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/node1宕机.webp %}
#### 扩容
按照规则，仅仅请求 Job_k 被重新分配到 Node_i 上去了，其它请求Job_1、Job_k+1、Job_i都不受影响，即**仅仅影响分流的很少部分请求**
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/一致性哈希扩容.webp %}
#### 引入虚拟节点
{% alert success no-icon%}
...
{% image fancybox fig-100  center  https://gitee.com/mingchaohu/blog-image/raw/master/image/经典负载均衡架构.webp %}

### HTTP 重定向负载均衡

重定向服务器仅仅做一件事情：在响应头中写入重定向地址，利用路由算法，改写请求的目标 IP 地址，并把 IP 地址写入到响应的重定向 Head 中，随后把请求发送到用户端。用户端解析重定向响应，把请求重新发送给 Web 应用服务器的目标服务器

{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/http重定向负载均衡.webp %}
:sparkles:特点：
...
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/dns负载均衡.webp %}
...
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/二级负载均衡.webp %}
### 反向代理负载均衡
...
{%endalert%}
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/反向代理负载均衡.webp %}
...
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/ip负载均衡.webp %}
...
这也是在生产环境中，大型互联网所采用的
{%endalert%}
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/数据链路层负载均衡.webp %}
## 高可用系统
...
{%endalert%}
{% image fancybox fig-100  center  https://gitee.com/mingchaohu/blog-image/raw/master/image/分层负载均衡.webp %}
接下来将对每一层的负载均衡方案进行介绍
### 第一层：客户端层到反向代理层
{% alert success no-icon%}
...
{%endalert%}
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/客户端到反向代理层的负载均衡.webp %}
### 第二层：反向代理层到站点层
{% alert success no-icon%}
...
{%endalert%}
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/反向代理到站点.webp %}
...
{%endalert%}
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/站点层到服务层.webp %}
### 第四层：访问数据层
{% alert success no-icon%}
...
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/按照range水平切分.webp %}
#### 按照id哈希水平切分
{% alert success no-icon%}
...
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/按照id哈希水平切分.webp %}
### 总结
...
{% image fancybox fig-100  center https://gitee.com/mingchaohu/blog-image/raw/master/image/负载均衡总体架构.jpg %}
